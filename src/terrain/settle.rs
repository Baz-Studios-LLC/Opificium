...
use crate::terrain::ground::Recipe;
...
impl Settlements {
    ...
    pub fn plan(
        ...
        for (wanted, city) in [(recipe.cities, true), (recipe.cities + recipe.towns, false)] {
            ...
            while sites.len() < wanted && tries < 20_000 {
                ...
                tries += 1;
                let at = Vec2::new(
                    (unit(recipe.seed, n * 2) * 2.0 - 1.0) * half.x * 0.94,
                    (unit(recipe.seed, n * 2 + 1) * 2.0 - 1.0) * half.y * 0.94,
                );

                // Inland of the beach, so levelling a site never eats a shore.
                if shore(at) < recipe.site_min_inland {
                    continue;
                }
                let height = ground(at);
                // Not up a mountain. People build where the living is.
                if height > recipe.site_max_height {
                    continue;
                }
                // Not on ground that is already a hillside: levelling that would
                // leave a scar you could see from orbit.
                if steepness(ground, at) > recipe.site_max_slope {
                    continue;
                }
                if sites
                    .iter()
        ...
        settlements.index(recipe);
        settlements
    }

    /// Files every feature into the cells its reach touches.
    fn index(&mut self, recipe: &Recipe) {
        ...
        for (i, site) in self.sites.iter().enumerate() {
            let reach = site.radius + recipe.site_skirt;
            filings.push((i as u16, site.at - reach, site.at + reach));
        }
        let road_reach = recipe.road_width + recipe.road_skirt;
    ...
    pub fn level(&self, at: Vec2, recipe: &Recipe) -> Option<(f32, f32)> {
        ...
        for &what in cell {
            let (height, pull) = if what < sites {
                ...
                (
                    site.height,
                    smoothstep(site.radius + recipe.site_skirt, site.radius, away),
                )
            } else {
