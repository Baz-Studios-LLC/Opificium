...
//!
//! # It knows nothing about Opificium
//!
//! No bench, no palette, no project — a grid and a brush and a file. The same is
//! true of `chunk` and most of `ground`. What is left in `mod` is the part that
...
/// What the sculpted file is called, inside the project's world folder.
...
impl Sculpt {
    pub fn new(half: Vec2, seed: u32) -> Self {
    ...
    pub fn load(folder: &Path, half: Vec2, seed: u32) -> Self {
        Self::load_from(&Self::road(folder), half, seed)
    }

    pub fn load_from(road: &Path, half: Vec2, seed: u32) -> Self {
        let mut empty = Self::new(half, seed);
        if !road.exists() {
            return empty;
        }

        let bytes = match fs::read(road) {
            Ok(bytes) => bytes,
            Err(why) => {
        ...
        if bytes.len() < header || &bytes[..8] != MAGIC {
            warn!("{} is not sculpted ground - left alone", road.display());
            return empty;
        }
        ...
        if wide != empty.wide || deep != empty.deep || kept_half.distance(half) > 1.0 {
            warn!(
                "{} was sculpted for a {:.0}x{:.0} m world and this one is {:.0}x{:.0} m \
                 - left alone rather than put in the wrong place",
                road.display(),
                kept_half.x * 2.0,
                kept_half.y * 2.0,
                half.x * 2.0,
        ...
        if bytes.len() < header + wide * deep * 4 {
            warn!("{} is cut short - left alone", road.display());
            return empty;
        }

        empty.offsets = (0..wide * deep).map(|i| real(header + i * 4)).collect();
        // Counted once here; `write` keeps it current from now on.
    ...
    pub fn save(&mut self, folder: &Path) -> io::Result<PathBuf> {
        let road = Self::road(folder);
        self.save_to(&road)?;
        Ok(road)
    }

    pub fn save_to(&mut self, road: &Path) -> io::Result<()> {
        if let Some(above) = road.parent() {
            fs::create_dir_all(above)?;
        }

        let mut file = fs::File::create(road)?;
    ...
    fn ground_of(&self, values: &HashMap<usize, f32>) -> Rect {
        ...
        Rect::from_corners(low - CELL, high + CELL)
    }
    ...
    pub fn ramp(&mut self, from: Vec3, to: Vec3, width: f32, under: &dyn Fn(Vec2) -> f32) -> Rect {
        ...
        if length < 1.0 {
            return Rect::from_corners(start, start);
        }
...
mod tests {
    ...
    fn one_undo_takes_back_a_whole_drag_and_redo_puts_it_back() {
        ...
        assert!(changed.width() > 0.0 && changed.height() > 0.0);
    ...
    fn sculpted_ground_survives_being_written_and_read() {
        let road = std::env::temp_dir().join("opificium-sculpt-roundtrip.bin");
        let _ = fs::remove_file(&road);

        let mut kept = Sculpt::new(HALF, SEED);
        kept.apply(&stamp(
            Vec2::new(-100.0, 50.0),
            70.0,
            Brushing::Raise,
        ...
        kept.save_to(&road).expect("it should write");
        assert!(!kept.unsaved, "writing clears the unsaved mark");

        let read = Sculpt::load_from(&road, HALF, SEED);
        ...
        // A file sculpted for a different world must be refused, not stretched.
        let wrong = Sculpt::load_from(&road, HALF * 2.0, SEED);
        assert_eq!(wrong.sculpted_cells(), 0);
